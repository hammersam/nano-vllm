...
def act_quant_kernel(N, in_dtype=BF16, out_dtype=FP8, scale_dtype=FP32, round_scale=False):
    ...
    def act_quant_kernel_(
        ...
        with T.Kernel(T.ceildiv(M, blk_m), T.ceildiv(N, group_size), threads=128) as (pid_m, pid_n,):
            ...
            # register
            # 会将两个fp16输入值打包存入32bit register中，因此每个
            # 线程需要32 * 2 / 4 = 16个寄存器
            ...
            # 如果需要的话(num_stages > 1)，重叠数据加载和计算
            for _ in T.Pipelined(1, num_stages=num_stages):
                # 线程块协作将数据从hbm加载到shared mem
                T.copy(X[pid_m * blk_m, pid_n * group_size], x_shared)
                # 每个线程将自己需要的数据从shared mem复制到私有寄存器中
...
# 这个kernel并不是用来计算sparse mla indexer fp8 logits，而是用于linear layer
@tilelang.jit(pass_configs=pass_configs)
    ...
    def fp8_gemm_kernel_(
        ...
        with T.Kernel(
            ...
            for k in T.Pipelined(K_iters, num_stages=4):
                ...
                # Promote to enable 2xAcc(fp32高效累加)
...
def fp8_index_kernel(h: int, d: int):
    ...
    def fp8_index_kernel_(
        ...
        # 这里假设按照默认情况下head_dim = q_block_size = 128
        ...
        with T.Kernel(b, m, T.ceildiv(n, blk_n1)) as (i_b, i_m, i1_n):
            ...
            for i2_n in T.Pipelined(blk_n1 // blk_n2, num_stages=2):
                ...
                # k_smem: (blk_n2, d) @ q_smem: (d, h) -> (blk_n2, h)
                T.gemm(
                    k_smem,
                    q_smem,
                    logits,
...
def fp8_index(
    ...
    Args:
        ...
        q_s (torch.Tensor): The scaling factor for Q (float32), must be contiguous.
        k (torch.Tensor): The K tensor, must be contiguous.
        k_s (torch.Tensor): The scaling factor for K (float32), must be contiguous.

        fp8 q @ fp8 k -> fp32 logits
        relu(fp32 logits) * q_s (weights) -> fp32 logits
        fp32 logits -> fp32 logits_sum
        fp32 logits_sum * k_s (float32) -> fp32 index_score
    """
