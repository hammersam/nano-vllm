...
def generate(model: Transformer,
    ...
    tokens = torch.full((num_prompt, total_len), -1, dtype=torch.long, device="cuda")
    ...
    for cur_pos in range(min(prompt_lens), total_len):
        # (num_prompt, vocab_size)
        # 即使某个sample已经生成了eos，模型仍会把eos（以及之后生成的token）
        # 作为输入继续生成后续的token，这些在eos之后生成的token会在之后阶段
        # 被丢弃，不影响最终结果
        ...
        # 当前cur_pos如果没有prompt token就保留新生成的token
        # (num_prompt,)
        next_token = torch.where(prompt_mask[:, cur_pos], tokens[:, cur_pos], next_token)
        tokens[:, cur_pos] = next_token
        # 不能是prompt token并且生成的token是eos_id，满足这两个条件一个sample的生成才算作结束
        finished |= torch.logical_and(~prompt_mask[:, cur_pos], next_token == eos_id)
        prev_pos = cur_pos
        # 只有所有样本最新生成的token都是eos才停止继续生成
        if finished.all():
            break
    completion_tokens = []
...
def main(
    ...
    if interactive:
        ...
        while True:
            ...
            # 把聊天历史转化为token序列，并且在最后添加一个提示模型开始生成的token(例如<|assistant|>)
