...
class ParallelEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, dim: int):
        ...
        # 局部word embedding weights，只负责一部份vocabulary
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if world_size > 1:
            ...
            # 转化为局部token idx，与局部word embedding weights保持一致
            x = x - self.vocab_start_idx
            # 去掉这个gpu不负责处理的token indices
            x[mask] = 0
        # x: (b, s)
        # y: (b, s, dim)
        y = F.embedding(x, self.weight)
        if world_size > 1:
            # 需要把这个gpu不负责的token embeddings设置成0
            y[mask] = 0
            # 把所有gpu上的局部计算结果加起来即可
            dist.all_reduce(y)
        return y
...
def linear(x: torch.Tensor,
    ...
    Notes:
        - If `weight` is not quantized (i.e., `weight.dtype != fp8_e4m3` or `element_size() != 1`), 
          a dequantized version is used for computation.
...
class Linear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype=None):
        ...
        # if byte per element is 1, i.e. fp8_e4m3 quant
...
class RowParallelLinear(Linear):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # 输入x通常是已经被切分过了，例如来自前一个ColumnParallelLinear的输出
        # x: (b, s, in_features // N)
        # self.weight: (out_features, in_features // N)
        # y: (b, s, out_features)
...
def precompute_freqs_cis(args: ModelArgs) -> torch.Tensor:
    ...
    if seqlen > args.original_seq_len:
        ...
        # smooth是一个slop tensor，小于low为1，大于high为0，
        # 在这两者之间会从1线性下降到0
        # (qk_rope_head_dim // 2,)
        smooth = 1 - linear_ramp_factor(low, high, dim // 2)
        # 低维(低频)部分旋转频率不变，高维(高频)部分除以factor导致旋转频率变小
        # 波长变长，而smooth值在0到1之间的中间部分会在"保持不变"和"完全缩放"
        # 之间进行平滑的线性插值
        freqs = freqs / factor * (1 - smooth) + freqs * smooth

    t = torch.arange(seqlen)
    # 计算出每个位置/维度的具体旋转角度(position * frequency)
    freqs = torch.outer(t, freqs)
    # 将角度转化为复数形式 cos(angle) + i*sin(angle)
    # 之后apply_rotary_embedding会将q_pe, k_pe视为复数乘以
    # freqs_cis中取出的旋转复数，从而实现对q, k的旋转，添加位置编码信息
...
class Indexer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor,
        ...
        # 使用mla传来的compressed query来构造Indexer内部使用的q
        q = self.wq_b(qr)
        # 默认情况下n_heads = 64, head_dim = 128
        q = rearrange(q, 'b s (h d) -> b s h d', d=self.head_dim)
        q_pe, q_nope = torch.split(
            q, [self.rope_head_dim, self.head_dim - self.rope_head_dim], dim=-1)
        q_pe = apply_rotary_emb(q_pe, freqs_cis)
        q = torch.cat([q_pe, q_nope], dim=-1)
        # 只有一个key head, mqa style, 减少k_cache和k_scale_cache的大小
        # (b, s, head_dim)
        k = self.wk(x)
        k = self.k_norm(k)
        ...
        # 使用hadamard变换将outlier激活值进行均摊(缩小数值范围)，为了之后的量化做准备
        ...
        # q_scale: (b, m, h, d // block_size), 因为默认情况下
        # head_dim等于block_size，所以最后一个维度是1
        weights = weights.unsqueeze(-1) * q_scale * self.softmax_scale
        # weights: (b, m, h, d // block_size)
        # q: (b, m, h, d)
        # k: (b, n, d)
        # index_score: (b, m, n)
        ...
        # 验证本地计算结果是否和gpu0计算结果相同，正常情况下应该是相同，
        # 这里是为了捕捉不确定性(因为低精度、自定义的cuda kernel导致
        # 的数值差异)
        assert torch.all(topk_indices == topk_indices_), f"{topk_indices=} {topk_indices_=}"
        # 确保所有gpu上的topk_indices都是相同的才继续进行
        return topk_indices
...
class MLA(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # 如果模型配置的最大序列长度(max_seq_len)大于模型训练序列长度(original_seq_len)，
        # 放大softmax_scale补偿因修改RoPE位置编码以适应长文本而导致的logits分布变化
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor,
        ...
        """
        bsz, seqlen, _ = x.size()
        # start_pos表示当前输入x对应的kv应该存放在缓存中的开始位置，
        # 同时也是当前缓存中kv的数量，prefill下是0，decode下
        # end_pos = start_pos + 1，因为输入x的seq_len为1
        end_pos = start_pos + seqlen
        # compressed query
        ...
        kv = self.wkv_a(x)
        # kv: (B, T, kv_lora_rank)
        # k_pe: (B, T, qk_rope_head_dim)
        kv, k_pe = torch.split(kv, [self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
        kv = self.kv_norm(kv)
        k_pe = apply_rotary_emb(k_pe.unsqueeze(2), freqs_cis)
        # TODO: use fp8 kvcache, perform attention computation and store output in fp16 
        self.kv_cache[:bsz, start_pos:end_pos] = kv
        self.pe_cache[:bsz, start_pos:end_pos] = k_pe.squeeze(2)
        if mask is not None:  # MHA prefill, 模型需要一次性处理整个输入序列(一般seq_len > 1)
            # 在prefill和training中，mask是一个因果掩码，被应用到Indexer和注意力计算中
            # q: (B, T, n_local_heads, qk_nope_head_dim + qk_rope_head_dim)
            q = torch.cat([q_nope, q_pe], dim=-1)
            # 直接将wkv_b作用于整个kv张量，在gpu上非常高效
            # kv: (B, T, kv_lora_rank)
            kv = self.wkv_b(kv)
            kv = kv.view(bsz, seqlen, self.n_local_heads, self.qk_nope_head_dim + self.v_head_dim)
            # k_nope: (B, T, n_local_heads, qk_nope_head_dim)
            # v: (B, T, n_local_heads, v_head_dim)
            k_nope, v = torch.split(kv, [self.qk_nope_head_dim, self.v_head_dim], dim=-1)
            # k: (B, T, n_local_heads, qk_nope_head_dim + qk_rope_head_dim)
            k = torch.cat([k_nope, k_pe.expand(-1, -1, self.n_local_heads, -1)], dim=-1)
            # sdpa
            scores = torch.einsum("bshd,bthd->bsht", q.float(), k.float()) * self.softmax_scale
            # indexer
            # causal mask同样也被传递给了Indexer，被加到index_score上然后再去
            # 进行topk操作，确保Indexer在为每个query token寻找最相关的key token
            # 时也遵循causal关系，不会去索引未来的token
            # topk_indices: (B, T, top_k)
            topk_indices = self.indexer(x, qr, start_pos, freqs_cis, mask)
            index_mask = torch.full((bsz, seqlen, seqlen), float("-inf"),
                                    device=x.device).scatter_(dim=-1, index=topk_indices, src=0)
            index_mask += mask
            scores += index_mask.unsqueeze(2)
            scores = scores.softmax(dim=-1, dtype=torch.float32)
            # 降回bf16进行计算
            x = torch.einsum("bsht,bthd->bshd", scores.type_as(x), v)
        else:  # MHA decode, 用到了kvcache，模型每次只处理一个token(seq_len = 1)，利用所有之前的token缓存信息来生成下一个token
            # decoding阶段，模型每次只处理一个token，通过kvcache的机制来保证causality，
            # 即每个新生成的token只能关注包括自己在内的所有已经生成的token的缓存信息
            # 如果需要的话，dequantize key/value上投影部分的权重
            if self.dequant_wkv_b is None and self.wkv_b.scale is not None:
                self.dequant_wkv_b = weight_dequant(self.wkv_b.weight, self.wkv_b.scale)
            wkv_b = self.wkv_b.weight if self.dequant_wkv_b is None else self.dequant_wkv_b

            # NOTE: wkv_b在prefill和decode中使用方式不同，在decode阶段拥有end_pos个历史token的
            # kv缓存，如果像prefill一样每次生成一个token都需要把所有的kv取出来然后通过wkv_b投影成k
            # 和v之后再去计算注意力，计算浪费巨大
            ...
            scores += index_mask.unsqueeze(2)
            scores = scores.softmax(dim=-1, dtype=torch.float32)
            # (bs, seq_len, n_heads, end_pos) @ (bs, end_pos, kv_lora_rank) ->
            #   (bs, seq_len, n_heads, kv_lora_rank)
            x = torch.einsum("bsht,btc->bshc", scores.type_as(x), self.kv_cache[:bsz, :end_pos])
            # (bs, seq_len, n_heads, kv_lora_rank) @ (n_heads, v_head_dim, kv_lora_rank) -> 
            #   (bs, seq_len, n_heads, v_head_dim)
