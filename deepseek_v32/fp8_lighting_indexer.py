...
def mqa_attn_return_logits(
    ...
    def mqa_attn_return_logits_kernel(
            ...
            # CuSeqLenKS: key start index for each query (inclusive)
            # CuSeqLenKE: key end index for each query (exclusive)
            # Together define the valid attention window for each query token.
        ...
        with T.Kernel(T.ceildiv(seq_len, block_Q), threads=threads) as bx:
            ...
            # 计算整个query block的valid attention window范围(对block内所有
            # query的有效注意力窗口进行叠加)，会让query block与整合之后的注意力窗口
            # 内所有key计算注意力分数
            ...
            for nbn_i in T.Pipelined(
                ...
                # 虽然block_N中有些key可能不位于block_Q中某个query的valid attention window之内，但是位于
                # 整个query block的有效注意力窗口之内，因此仍然把注意力分数计算出来并进行存储，之后再做处理
...
def clean_logits_(
    ...
    # 这个kernel应该是在计算完indexer logits之后(mqa_attn_return_logits)调用，
    # 用于将每个query非valid attn window位置的logits置为负无穷
    @T.prim_func
    def clean_logits_kernel(
        ...
        with T.Kernel(seq_len, threads=threads) as bx:
            # 将tx绑定到threadIdx.x上，范围是[0, threads)
